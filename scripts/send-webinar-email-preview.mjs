...
function computeFireAt(step, base) {
  if (!base) return null;
  ...
  const d = kstBase.getUTCDate() + step.offsetDays;
  return new Date(Date.UTC(y, mo, d, hh, mm) - 9 * 60 * 60 * 1000);
}
...
const VARS = {
  ...
  "{{preQuestionUrl}}":
    campaign.preQuestionUrl || settings.webinar_pre_question_url || `${SITE}/webinar/ask/${campaign.id}`,
...
for (let i = 0; i < STEPS.length; i++) {
  ...
  try {
    ...
    if (error) {
      console.log(`  ${i + 1}. ❌ ${s.label} — ${error.message || JSON.stringify(error)}`);
      fail++;
    } else {
      console.log(`  ${i + 1}. ✅ ${s.label} — ${data?.id}`);
      ok++;
    }
  } catch (e) {
    console.log(`  ${i + 1}. ❌ ${s.label} — ${e.message}`);
    fail++;
  }
  // Resend rate limit 2req/sec → 여유있게 1.2초 대기
