/**
 * 가장 최근 캠페인의 카톡 BroadcastDraft를 지우고 최신 카피로 다시 시드.
 * skipPast: 24h 이상 지난 것은 자동 스킵.
 *
 * 실행: node scripts/reseed-kakao-broadcasts.mjs [campaignId]
 */
...
const c = campaignId
  ? await prisma.webinarCampaign.findUnique({ where: { id: campaignId } })
  : await prisma.webinarCampaign.findFirst({ orderBy: { updatedAt: "desc" } });
if (!c) { console.error("캠페인 없음"); process.exit(1); }
...
function computeFireAt(msg) {
  const base = msg.kind === "webinar" ? c.webinarDate : c.endDate;
  if (!base) return null;
  const [hh, mm] = msg.time.split(":").map(Number);
  const kstBase = new Date(base.getTime() + 9 * 3600000);
  const y = kstBase.getUTCFullYear();
  const mo = kstBase.getUTCMonth();
  const d = kstBase.getUTCDate() + msg.offsetDays;
  return new Date(Date.UTC(y, mo, d, hh, mm) - 9 * 3600000);
}

const VARS = {
  webinarDate: formatKstDate(c.webinarDate),
  zoomUrl: c.zoomUrl || settings.webinar_zoom_url || "[Zoom URL 미설정]",
  ebook1Url: settings.ebook1_url || settings.ebook_url || "[1차 전자책 URL 미설정]",
  preQuestionUrl: c.preQuestionUrl || settings.webinar_pre_question_url || `${SITE}/webinar/ask/${c.id}`,
  salesUrl: c.salesUrl || settings.external_checkout_url || "[강의 신청 URL 미설정]",
  consultationUrl: settings.consultation_url || `${SITE}/consultation`,
  replayUrl: c.replayUrl || c.zoomUrl || "[다시보기 URL 미설정]",
};
...
// lib/crm/kakao-broadcast-preset.ts 와 동일한 12개
const MESSAGES = [
  { kind:"webinar", offsetDays:-10, time:"09:00", label:"D-10 · 환영·대상자",
  ...
  { kind:"webinar", offsetDays:-7, time:"09:00", label:"D-7 · 라이브 후 변화 3가지",
  ...
  { kind:"webinar", offsetDays:-5, time:"09:00", label:"D-5 · 자유·시간·자동화",
  ...
  { kind:"webinar", offsetDays:-4, time:"09:00", label:"D-4 · 페르소나 + 공감·권유",
  ...
  { kind:"webinar", offsetDays:-3, time:"09:00", label:"D-3 · 다시보기 답변 3가지",
    body:`팁스타그램입니다.\n\n"라이브 못 볼 것 같은데 다시보기 되나요?"라는 질문에 답해드리겠습니다.\n\n답변에 앞서 세 가지만 꼭 말씀드립니다.\n\n1. 라이브는 처음이 가장 중요합니다.\n초반 30분에 알고리즘·구조·실행 핵심을 다 담아둡니다. 가능하면 시작 시간에 맞춰 들어와 주세요.\n\n2. 끝까지 들으시면 특별한 혜택이 있습니다.\n참여자 한정으로 65만 원 상당의 실전 자료 세트를 무료로 드립니다. 다시보기로는 받으실 수 없어요.\n\n3. 그래도 놓치신 분을 위한 안전장치.\n참여 신청하신 분께는 72시간 동안 다시보기 링크를 보내드립니다.\n\n정리하면 — 가능하면 시작 시간에 오시고, 끝까지 함께해 주세요.\n\n팁스타그램 드림` },
  { kind:"webinar", offsetDays:-2, time:"09:00", label:"D-2 · 질문 짧은 답 + 진단 세션",
  ...
  { kind:"webinar", offsetDays:-1, time:"09:00", label:"D-1 · 내일 · 진단 세션",
  ...
  { kind:"endDate", offsetDays:-3, time:"09:00", label:"마감 D-3 · 5자리 남음",
    body:`팁스타그램입니다.\n\n이번 기수 모집 마감까지 3일 남았습니다.\n\n이번 기수는 20명 한정입니다. 진도 확인과 피드백을 직접 드리는 구조라 더 늘리기 어렵습니다.\n\n지금 5자리 남았습니다.\n\n3일 후에 이번 기수 모집을 닫습니다.\n\n아직 망설이고 계시다면 남은 자리만 한 번 확인해 보세요.\n\n▶ 신청 페이지\n{{salesUrl}}\n\n팁스타그램 드림` },
  { kind:"endDate", offsetDays:-1, time:"09:00", label:"마감 D-1 · 변화 vs 정체",
...
const now = new Date();
let created = 0, skippedPast = 0;
console.log(`캠페인: ${c.name}`);
console.log(`webinarDate: ${VARS.webinarDate}\n`);

for (let i = 0; i < MESSAGES.length; i++) {
  const m = MESSAGES[i];
  const fireAt = computeFireAt(m);
  if (!fireAt) continue;
  if (now.getTime() - fireAt.getTime() > 24 * 3600000) {
    skippedPast++;
    console.log(`  ⏰ skip past (${fireAt.toISOString()}) | ${m.label}`);
    continue;
  }
  await prisma.broadcastDraft.create({
    data: {
      ...
      body: applyVars(m.body),
      scheduledAt: fireAt,
      status: "scheduled",
      notes: `[campaign:${c.id}:step:${i}]`,
    },
  });
  created++;
  console.log(`  ✓ ${fireAt.toISOString()} | ${m.label}`);
}

console.log(`\n생성 ${created}개 · 지난 시각 스킵 ${skippedPast}개`);
