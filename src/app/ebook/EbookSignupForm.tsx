...
export function EbookSignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ebook/step1", {
        method: "POST",
  ...
  if (done) {
    return (
      <div className="text-center py-6">
        ...
        <h2 className="text-xl sm:text-2xl font-black text-white mb-3">신청이 완료됐습니다 🎉</h2>
        <p className="text-white/70 text-sm sm:text-base leading-relaxed">
          입력하신 이메일 <strong className="text-white">{email}</strong> 로<br/>
          1차 전자책 다운로드 안내를 보내드렸습니다.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="홍길동"
            autoComplete="name"
            className="w-full pl-11 pr-4 py-3.5 rounded-xl bg-white/[0.06] border border-white/10 text-white placeholder:text-white/30 text-sm focus:outline-none focus:border-pink-400 focus:bg-white/[0.08] transition-colors"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-white/40 mt-1.5">이 이메일로 전자책 다운로드 안내를 보내드립니다.</p>
      </div>
      ...
      <label className="flex items-start gap-3 cursor-pointer pt-1">
        ...
        <span className="text-xs text-white/65 leading-relaxed">
          무료 전자책 및 관련 콘텐츠 발송 목적의 개인정보 수집·이용에 동의합니다.
          이름·이메일은 안내 목적으로만 사용되며, 신청자가 요청 시 즉시 파기됩니다.
        </span>
      </label>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full inline-flex items-center justify-center gap-2 py-4 rounded-xl ig-gradient text-white font-bold text-base shadow-lg shadow-pink-900/30 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
      >
        {loading ? (
          <>
