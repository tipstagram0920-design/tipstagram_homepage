...
export function FreebieSignupForm({ slug }: { slug: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/freebie/submit", {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-white/40 mt-1.5">이 이메일로 자료 다운로드 안내를 보내드립니다.</p>
      </div>
      ...
      <label className="flex items-start gap-3 cursor-pointer pt-1">
        ...
        <span className="text-xs text-white/65 leading-relaxed">
          무료 자료 및 관련 콘텐츠 발송 목적의 개인정보 수집·이용에 동의합니다.
        </span>
      </label>
