...
[릴스파이에서 찾아볼 참고 계정]
- 학생의 주제·소비자 문제에 맞춰, 릴스파이(reelspy)에서 검색해볼 키워드·카테고리 3~5개를 제안한다.
- 존재하지 않을 수 있는 특정 인스타 계정 아이디는 지어내지 말 것. 대신 "이런 키워드로 검색해 상위 릴스를 올린 계정을 벤치마킹하라"는 식으로 안내한다.
- 무엇을 관찰할지(후킹 방식, 편집 템포, 자막 스타일 등)도 한 줄 덧붙인다.
