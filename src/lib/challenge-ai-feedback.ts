...
interface Week4FormData {
  kind?: string;
  reels?: { refUrl?: string; myUrl?: string; note?: string }[];
}
interface Week3FormData {
  kind?: string;
  reels?: { url?: string; note?: string }[];
}
...
export function buildSubmissionTranscript(content: string, formData: unknown): string {
  const fd = (formData ?? null) as
    | (Week1FormData & Week2FormData & Week3FormData & Week4FormData)
    | null;

  if (fd?.kind === "week4_reels_5") {
    const w4 = fd as Week4FormData;
    const reels = (w4.reels ?? []).filter((r) => r?.refUrl || r?.myUrl);
    const blocks: string[] = [
      "이번 주 숙제: 잘 된 릴스 레퍼런스 5개를 찾아, 각각을 자기 콘텐츠로 변형해 릴스 5개를 만들어 올리고 레퍼런스 URL과 내 릴스 URL을 짝지어 제출하는 것.",
      "· 평가 관점: (1) 5개를 실제로 채웠는가(실행량) (2) 레퍼런스를 그대로 베낀 게 아니라 자기 상품·고객 이야기로 변형했는가 (3) 레퍼런스에서 '왜 잘 됐는지'의 핵심(첫 3초·구성·자막)을 제대로 가져왔는가 (4) 5개가 서로 다른 각도를 시도했는가, 같은 포맷만 반복했는가 (5) 1주차에 정의한 소비자·상품과 이어지는가.",
      "· 피드백에는 반드시 포함할 것: 릴스마다 레퍼런스와 내 릴스를 비교해 개별 코멘트를 주고, 다음 주에 무엇을 하나만 바꾸면 되는지 한 줄 액션을 제시한다.",
      `· 제출 개수: ${reels.length}개 (요구 5개)`,
    ];
    reels.forEach((r, i) => {
      blocks.push(
        [
          `[릴스 ${i + 1}]`,
          `- 참고 레퍼런스: ${r.refUrl?.trim() || "(미입력)"}`,
          `- 내 릴스: ${r.myUrl?.trim() || "(미입력)"}`,
          `- 학생이 적은 변형 포인트: ${r.note?.trim() || "(미작성)"}`,
        ].join("\n")
      );
    });
    return blocks.join("\n\n").trim();
  ...
  if (fd?.kind === "week3_reels_upload") {
    const w3 = fd as Week3FormData;
    const reels = (w3.reels ?? []).filter((r) => r?.url);
    const blocks: string[] = [
      "이번 주 숙제: 인스타그램에 릴스를 최소 3개 올리고, 각 릴스의 URL을 제출하는 것. 이번 주는 '양'을 만드는 주차다.",
      "· 평가 관점: (1) 최소 개수를 실제로 채웠는가(실행량) (2) 각 릴스의 대상과 첫 3초 후킹이 뚜렷한가 (3) 1주차에 정의한 소비자·상품과 릴스 주제가 이어지는가 (4) 3개가 서로 다른 각도를 시도했는가, 아니면 같은 걸 반복했는가.",
      "· 피드백에는 반드시 포함할 것: 각 릴스 URL마다 개별 코멘트를 주고(URL과 학생이 적은 주제·후킹을 근거로), 다음 주에 무엇을 하나만 바꾸면 되는지 한 줄 액션을 제시한다.",
      `· 제출 개수: ${reels.length}개 (요구 최소 3개)`,
    ];
    reels.forEach((r, i) => {
      blocks.push(
        [
...
- 항목별 피드백 다음에 "총평:" 으로 시작하는 3~4문장: 전체적으로 잘한 점, 항목 간 일관성, 다음 주 한 줄 액션.

총평 뒤에 아래 순서로 마무리한다:

[릴스 후킹 제목 10개]
- 학생이 정의한 '소비자의 문제'(Q3·Q9 등)에 딱 맞는, 실제로 만들어볼 만한 릴스 첫 3초 후킹 제목 10개를 번호로 제시한다.
- 문제를 정면으로 건드리는 후킹(예: "○○ 때문에 매번 실패하셨죠?"), 반전·호기심·구체적 수치 등 다양한 유형을 섞는다.
- 학생의 실제 주제·표현을 반영해 바로 촬영할 수 있게 구체적으로.

그리고 마지막으로, 섹션 제목 없이 학생을 따뜻하게 독려하는 한두 문장으로 끝맺는다. (앞으로의 가능성을 믿어주고 이번 주 노력을 인정하는 응원. 이 격려 문장이 피드백의 가장 마지막이 되어야 한다.)

- 각 항목·섹션 사이는 빈 줄 하나로 구분. 마크다운 기호(#, *, - 등) 쓰지 말고 자연스러운 문장으로(번호 목록은 "1." 형태 허용).
